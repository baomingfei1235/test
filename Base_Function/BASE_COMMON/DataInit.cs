...
namespace Base_Function.BASE_COMMON
{
    ...
    public class DataInit
    {
        ...
        static DataInit()
        {
            if (EmrCommon.DbFunc.funcTable == null)
                EmrCommon.DbFunc.funcTable = o => App.GetDataSet(o)?.Tables[0];
        }
        ...
        /// 初始化病人信息
        /// </summary>
        /// <param name="oldInpatient">当前病人信息</param>
        ...
        /// 根据病人id得到病人信息
        /// </summary>
        /// <param name="patient">当前病人id</param>
        /// <returns>InPatientInfo对象</returns>
            ...
            try
            {
                ...
                if (group_id != 0)
                {
                    DataSet ds_group_tid = App.GetDataSet("select tid from t_tempplate a inner join T_TEMPPLATE_GROUP c on a.tid=c.template_id where a.text_type='" +
                                                           textId + "' and c.group_id='" + group_id + "' and c.ISDEFAULT='Y'");
                    string sql = "";
                    if (ds_group_tid != null)
                    {
                        ...
                        if (ds_group_tid.Tables[0].Rows.Count > 0)
                        {
                            sql = "select b.content from t_tempplate a" +
                                                     ...
                                                     " where c.isdefault='Y' and a.text_type='" + textId + "' and c.group_id=" + group_id + "";
                ...
                */
                    ...
                    foreach (XmlNode childNode in nodeList)
                    {
                        if (childNode.Attributes["timeTitle"] != null && childNode.Attributes["timeTitle"].Value == "Y")
                        {
                            ...
                            string[] arr = textTitle.Trim().Split(new string[] { "  " }, StringSplitOptions.RemoveEmptyEntries);
                            textTitle = string.Join("   ", arr);
                            break;
                        }
                        //else if (childNode.Attributes["timeTitle"] != null )
                        //{ 
                        //    textTitle = childNode.Attributes["title"] != null ? childNode.Attributes["title"].Value : "";
                        //    string[] arr = textTitle.Trim().Split(new string[] { "  " }, StringSplitOptions.RemoveEmptyEntries);
                        //    textTitle = arr[0]+"   "+ "病程记录";
                        //    break;
                        //}
                    }
                    if (textTitle != "")
                    ...
                    try
                    {
                        ...
                        if (CurrentFrmText.MyDoc.Us.Tid != 0)
                        {
                            ...
                            if (CurrentFrmText.MyDoc.Us.OldTextName == "病程记录") {
                                 sql = string.Format("update T_PATIENTS_DOC set HAVEDOCTORSIGN = '{3}' ,DOC_NAME='{2}',ISHIGHERSIGN = '{4}',HAVEHIGHERSIGN = '{5}',submitted='{6}',highersignuserid = '{7}',textname='{8}' where TID = {0} AND PATIENT_ID = '{1}' ",
                            ...
                            {
                                sql = string.Format("update T_PATIENTS_DOC set HAVEDOCTORSIGN = '{3}' ,ISHIGHERSIGN = '{4}',HAVEHIGHERSIGN = '{5}',submitted='{6}',highersignuserid = '{7}',textname='{8}' where TID = {0} AND PATIENT_ID = '{1}' ",
                              CurrentFrmText.MyDoc.Us.Tid,
                              CurrentFrmText.MyDoc.Us.InpatientInfo.Id,
                              textTitle == "" ? "" : "",
                              havaTube,
                            ...
                            if (App.ExecuteSQL(sql) > 0)
                            {
                                ...
                                if (blfalse)
                                {
                                    ...
                                    if (haveD && App.GetDataSet("SELECT PATIENT_ID from t_job_temp where operate_type='D型病历' and PATIENT_ID = '" + CurrentFrmText.MyDoc.Us.InpatientInfo.Id + "'").Tables[0].Rows.Count < 1)
                                    {
                                        ...
                                        string sqlD = string.Format("insert into v_job_temp(pid,operate_type,operate_time,patient_id,age) " +
                ...
                else
                {
                    ...
                    if (ds != null && ds.Tables[0].Rows.Count > 0)
                    {
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                            ...
                            string ctitlename = System.Text.RegularExpressions.Regex.Replace(ds.Tables[0].Rows[i]["ctitlename"].ToString().Trim(), @"\s", "");
                            ...
                            XmlNodeList sourcelist2 = ((XmlElement)sourcebodynode).GetElementsByTagName("input");
                            ...
                            foreach (XmlNode xn in sourcelist2)
                            {
        ...
        private static XmlNode ReadXml(string xml, string TitleName, bool isTitle, string InputName, bool isInput)
        {
            ...
            try
            {
                ...
                XmlNodeList list = ((XmlElement)bodynode).GetElementsByTagName("div");
                XmlNodeList list2 = ((XmlElement)bodynode).GetElementsByTagName("input");
                foreach (XmlNode xn in list)
                {
                    if (xn.Attributes["title"] != null)
                    {
        ...
        private static ZYTextElement s;
        ...
        internal static void Filter(XmlElement bodyElement, int patient_id)
        {
            ...
            try
            {
                if (bodyElement != null && patient_id > 0)
                {
                    ...
                    //体征信息 T、P、R等
                    Filter(xelements, macrosElements, "6", "select * from v_patient_tpr a where a.patient_id='" + patient_id + "'", true);

                    //其他体征信息BP等
                    Filter(xelements, macrosElements, "7", "select * from v_patient_other_vital a where a.patient_id='" + patient_id + "'", true);
                }
            }
            catch { }
        ...
        static void GetDiagnoseElements(ArrayList elements,List<ZYTextElement> list=null)
        {
            if (list == null)
                list = new List<ZYTextElement>();
            foreach (var item in elements)
            {
                 if(item is ZYTextInput)
                {
                    ZYTextInput input = (ZYTextInput)item;
                    if (input.Name.Contains("诊断"))
                        list.Add(input);
                }
        ...
        /// 刷新编辑器中诊断内容Action
        /// </summary>
        public static Action<int> RefreshDocDiagnoseAction = null;
        ...
        public static void RefreshDocDiagnose(frmText text, int Reftype)
        {
            try
            {
                ...
                //var elements=  text.MyDoc.Elements.Cast<ZYTextElement>().Where(o => o is ZYTextInput).Cast<ZYTextInput>().Where(o => o.Name.Contains("诊断"));
                //  text.MyDoc.DiagnoseDiv(text.MyDoc.DocumentElement, arrlist);
                var elements = new List<ZYTextElement>();
                GetDiagnoseElements(text.MyDoc.Elements, elements);
               ...
               if (elements.Any())
                ...
                {//获取到编辑器中div
                    ...
                    DateTime sysTime = App.GetSystemTime();
                    foreach (ZYTextElement element in elements.ToList())
                    {
                        if (element is ZYTextDiv)
                        {
                            ZYTextDiv div = (ZYTextDiv)element;
                        ...
                        else if (element is ZYTextInput)
                        {
                            ...
                            DataInit.getDivtitle(input.Name, ref name, ref type);
                            if (!string.IsNullOrEmpty(type) && !string.IsNullOrEmpty(name))
                            {//加载最新诊断内容到对应div
                                input.ChildElements.Clear();
                                input.CanDelete = false;
                                bool EnableTime = true;
                                //List<ZYTextElement> myList = Base_Function.BLL_DIAGNOSE.DiagnoseAction.GetDiagnoseToElement(text.MyDoc, type, title, divname, text.MyDoc.Us.InpatientInfo.Id.ToString(), EnableTime, TimeName);
                                var Diagnoses = Base_Function.BLL_DIAGNOSE.DiagnoseAction.GetDiagnose(text.MyDoc.Us.InpatientInfo.Id.ToString(), type);
                                if (Diagnoses?.Count > 0)
                                {
                                    var Diagnose = string.Join("；", Diagnoses.Select(o => o.DiagnoseName).ToArray());
                                    if(Diagnose != null)
                                    {
                                        Diagnose = Diagnose + "。";
                                    }
                                    ...
                                    //foreach (ZYTextElement elementE in myList)
                                    //{
                                    //    elementE.Parent = input; //div
                                    //    elementE.OwnerDocument = text.MyDoc;
                                    //    elementE.DeleterIndex = -1;
                                    //    elementE.CreatorIndex = text.MyDoc.SaveLogs.CurrentIndex;
                                    //    elementE.RefreshSize();
                                    //}
                                    //input.ChildElements.InsertRange(0, myList);
                                    input.Text = Diagnose;
        ...
        public static void getDivtitle(string divtitle, ref string title, ref string type)
        {
            ...
            else if (divtitle.Contains("目前诊断"))
            {
                type = "2147232996";
                title = "目前诊断";
            }
        ...
        public static void OpenPatientPath(int patient_id)
        {
            try
            {
                ...
                Type type = assembly.GetExportedTypes().ToList().FirstOrDefault(o => o.Name == classname);
        ...
        /// </summary>
        /// <param name="patient_id">患者ID</param>
        /// <param name="diagnose_code">诊断编码</param>
        /// <param name="diagnose_name">诊断名称</param>
        /// <param name="diagnose_type">诊断类型id</param>
