    ...
    public partial class ucFollowTobeFinished : UserControl
    {
        ...
        public DataTable SelectDataSet(Class_FollowInfo selectedInfo)
        {
            ...
            try
            {
                ...
                {
                //筛选科室
                    paTemp = "select ''序号,t.id 病人ID,t.pid 住院号,t.patient_name 病人姓名,t.age||t.age_unit  年龄,t.section_id 科室号,t.section_name 科室,t.sick_doctor_id 医生号,t.sick_doctor_name 管床医生,(select name from COVER_DIAGNOSE c where c.patient_id=t.id and c.id=(select max(id) from cover_diagnose where patient_id=c.patient_id)) 诊断,t.now_addres_phone 电话,t.now_address 地址,t.leave_time 参考时间,t.die_flag 死亡标记 from T_IN_PATIENT t where t.die_flag=0 and t.leave_time >to_date('" + Convert.ToDateTime(selectedInfo.Createtime.Substring(0,selectedInfo.Createtime.IndexOf(" "))).ToShortDateString() + "','yyyy-MM-dd')";
                }
                if (sMethod == 2)
                {
                    paTemp = "select ''序号,t.id 病人ID,t.pid 住院号,t.patient_name 病人姓名,t.age||t.age_unit  年龄,t.section_id 科室号,t.section_name 科室,t.sick_doctor_id 医生号,t.sick_doctor_name 管床医生,(select name from COVER_DIAGNOSE c where c.patient_id=t.id and c.id=(select max(id) from cover_diagnose where patient_id=c.patient_id)) 诊断,t.now_addres_phone 电话,t.now_address 地址,t.in_time 参考时间,t.die_flag 死亡标记 from T_IN_PATIENT t where t.die_flag=0 and t.leave_time > to_date('" + Convert.ToDateTime(selectedInfo.Createtime.Substring(0, selectedInfo.Createtime.IndexOf(" "))).ToShortDateString() + "','yyyy-MM-dd')";
                }
                if (sMethod == 3)
                {
                    paTemp = "select ''序号,t.id 病人ID,t.pid 住院号,t.patient_name 病人姓名,t.age||t.age_unit  年龄,t.section_id 科室号,t.section_name 科室,t.sick_doctor_id 医生号,t.sick_doctor_name 管床医生,(select name from COVER_DIAGNOSE c where c.patient_id=t.id and c.id=(select max(id) from cover_diagnose where patient_id=c.patient_id)) 诊断,t.now_addres_phone 电话,t.now_address 地址,max(oper_date) 参考时间,t.die_flag 死亡标记 from T_IN_PATIENT t join COVER_OPERATION o on t.id=patient_id where t.die_flag=0 and t.leave_time > to_date('" + Convert.ToDateTime(selectedInfo.Createtime.Substring(0, selectedInfo.Createtime.IndexOf(" "))).ToShortDateString() + "','yyyy-MM-dd')";
                }
                if (sMethod == 4)
                {
