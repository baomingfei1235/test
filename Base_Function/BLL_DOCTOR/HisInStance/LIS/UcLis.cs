    ...
    public partial class UcLis : UserControl
    {
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string Sql = @"select  m.patient_id HISID,t.pid 住院号,t.patient_name 姓名,m.RESULTS_RPT_DATE_TIME 检验日期,i.item_name 检验项目,t.card_id 身份证号,m.SPECIMEN 标本名称,d.name 申请人,'' 检验项目代码,m.test_no 标本流水号 
                                from lab_test_master@dbhislink m 
                                inner join t_in_patient t on m.patient_id=t.his_id 
                                inner join staff_dict@dbhislink d on d.emp_no=m.ORDERING_PROVIDER
                                inner join (select test_no,max(ITEM_NAME) ITEM_NAME from (select test_no,wm_concat(ITEM_NAME) over(partition BY test_no order by ITEM_NO) ITEM_NAME from lab_test_items@dbhislink where test_no in(select test_no from lab_result@dbhislink)) group by test_no) i on m.test_no=i.test_no 
                                where visit_id=1 and m.RESULTS_RPT_DATE_TIME is not null  and t.pid='" + txtPid.Text.Trim() + "'";
