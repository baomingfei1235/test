...
namespace Base_Function.BLL_DOCTOR
{
    public partial class ucDoctorOperater : UserControl
    {
        ...
        public ucDoctorOperater(InPatientInfo patientInfo)
        {
            ...
            this.c1OutBar1.SelectedIndex = 0;//左边默认选择'已写文书'
            ...
            if ((App.UserAccount.CurrentSelectRole.Role_type != "D" && App.UserAccount.CurrentSelectRole.Role_type != "N") || currentPatient.PatientState == "借阅")
            {
                 ...
                 */
                c1OutPage4.Enabled = false;
            }
            //加载联想输入库
            InitAutoCompleteCustomSource(txtSearchAllText);

            if (DataInit.RefreshDocDiagnoseAction == null)
                DataInit.RefreshDocDiagnoseAction = RefreshTabDocDiagnose;
        }
        ...
        public void AddFinishNode()
        {
            ...
            Bifrost.WebReference.Class_Table[] tablesqls = new Bifrost.WebReference.Class_Table[3];
            ...
            //////隐藏护理记录中定制文书
            DataInit.removeNodeCareDoc(advFinishDoc.Nodes, table_careDoc, null);
            ////所写文书的内容绑定到文书树上
        ...
        private bool CreateNewPage(ArrayList list, Node node)
        {
            ...
            if (page.Tag != null)
            {
                ...
                if (node.Tag.ToString().Contains("Class_Text"))
                {
                    ...
                    else if (ctext.Formname.ToLower() == "frmcases_first") //病案首页
                    {
                        InPatientInfo inpatient = page.Tag as InPatientInfo;
                        page.Tooltip = "N";
                        //重新过去最新信息
            ...
            if (NowTree.SelectedNode.Tag.GetType().ToString().Contains("Class_Text"))
            {
                    ...
                    NowTree.SelectedNode.Nodes[0].Tag.GetType().ToString().Contains("Patient_Doc"))
                {
                    ...
                    if (page.Tag != null)
                    {
                        ...
                        #endregion
                        string strxml = @"<div id='C8272494' marginLeft='10' candelete='1'  timeTitle='Y' name='' title='" + Record_Time + "                 " + Record_Content + "'><p operatercreater='0' /></div>";
                                            ...
                                            foreach (XmlNode bodyNode in xmlNode.ChildNodes)
                                            {
                                                if (bodyNode.Name == "body")
                                                {
                                                    if (select_text.Ishavetime != "")
                                                    {
                                                        ...
                                                        {//时间标题中隐藏时间项
                                                            bodyNode.InnerXml = strxml + bodyNode.InnerXml;
                                                        }
                                                        else
                                                        {
                                                            bodyNode.InnerXml = strxml + bodyNode.InnerXml;
        ...
        {
            if (c1OutBar1.SelectedPage.Text == "已写文书")
            {
                NowTree = advFinishDoc;
            }
            else if (c1OutBar1.SelectedPage.Text == "全部文书")
            {
                NowTree = advAllDoc;
            }
        }
        ...
        public void closefrmdiagnose(int n)
        {
            if (n == 2)
            {
                DataInit.RefreshDocDiagnoseAction(2);
            }
        }
