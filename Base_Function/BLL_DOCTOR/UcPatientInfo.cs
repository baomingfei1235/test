...
namespace Base_Function.BLL_DOCTOR
{
    ...
    public partial class UcPatientInfo : UserControl
    {
        ...
        private void InitForm()
        {
            ...
            try
            {
                cbxPay1.SelectedValue = this.inPatient.Pay_Manager.Length == 0 ? "0" : this.inPatient.Pay_Manager;
            }
            catch
            {
                cbxPay1.SelectedIndex = 0;
            }
            cbxPay.Text = this.inPatient.Pay_Manager;
            ...
            try
            {
                ...
                cbxMarred.SelectedValue = this.inPatient.Marrige_State;
            }
            catch
            {
            ...
            */
            ...
            try
            {
                ...
                cbxNational.SelectedValue = this.inPatient.Folk_code;
        ...
        private void btnOK_Click_1(object sender, EventArgs e)
        {
            ...
            */
            sb.Append(string.Format(@" birth_place='{0}',", txtXian.Text));
            ...
            */
            sb.Append(string.Format(@" native_place='{0}',", txtJGXian.Text));
            ...
            */
            sb.Append(string.Format(@" now_address='{0}',", txtNowXian.Text));
            ...
            // 医疗付款方式
            if (cbxPay1.SelectedValue != null && cbxPay1.SelectedValue.ToString() != "") 
            {
            ...
            {
                sb.Append(string.Format(@" marriage_state='{0}',", cbxMarred.Text));
            }
            else
            {
            ...
            {
                sb.Append(string.Format(@" folk_code='{0}',", cbxNational.Text));
            }
            else
            {
        ...
        /// 手动同步his病人基本信息
        /// </summary>
        private void SynHisPatientInfoByHisId(string zyh)
        {
            try
            {
                string sql = "select * from INTF_EMR_INPATIENTINFVIEW@dbhislink where zyh = '" + zyh + "'";

                DataSet dsHisPatient = App.GetDataSet(sql);
                DataTable dt = dsHisPatient.Tables[0];

                if (dt != null && dt.Rows.Count > 0)
                {
                    string UpdatePatient = "update T_IN_PATIENT set PATIENT_NAME='"
                                                                    + dt.Rows[0]["BRXM"].ToString() + "', InHospital_Count = '" + dt.Rows[0]["ZYCS"].ToString() + "', NAME_PINYIN='"
                                                                    + App.getSpell(dt.Rows[0]["BRXM"].ToString()) + "',GENDER_CODE='"
                                                                    + dt.Rows[0]["BRXB"].ToString() + "',BIRTHDAY=to_timestamp('" + dt.Rows[0]["CSNY"].ToString() + "','syyyy-mm-dd hh24:mi:ss.ff9')"
                                                                    + "',IN_TIME=to_timestamp('" + dt.Rows[0]["RYRQ"].ToString() + "','syyyy-mm-dd hh24:mi:ss.ff9')"
                                                                    + ",leave_time=to_timestamp('" + dt.Rows[0]["CYRQ"].ToString() + "','syyyy-mm-dd hh24:mi:ss.ff9')"
                                                                    + " where his_id='" + zyh + "'";
                    int num = App.ExecuteSQL(UpdatePatient);
                    if (num > 0)
                    {
                        this.inPatient = GetInPatientById(inPatient.Id.ToString()); // 重构病人实例
                        InitForm();
        ...
        {
            SynHisPatientInfoByHisId(this.inPatient.His_id);
            /*
            try
            {
